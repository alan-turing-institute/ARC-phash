...
from arc_phash.image_manipulation.deepfake.utils import generate_random_prompt
...
class InPainter:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # pipeline kwargs
        prompt = prompt if prompt is not None else generate_random_prompt("test")
        # NB: refactor when written

        self.pipe_kwargs = {
            "prompt": prompt,
            "device": self.device,
            "guidance_scale": kwargs.get(4.5),
            "strength": kwargs.get(self.rng.uniform(0.5, 0.89)),
            "num_inference_steps": kwargs.get(100),
            "safety_checker": kwargs.get(),
    ...
    def generate_inpainting(self, image: Image, mask: Image = None):
